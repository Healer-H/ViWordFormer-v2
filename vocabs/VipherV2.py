...
class VipherTokenizerV2:
    ...
    def _initialize_special_tokens(self, config) -> None:
        ...
        # Because we store each token as a quintuplet (onset, tone, medial, nucleus, coda),
        # we treat each special token as the same index for all five parts.
        self.pad_idx = (
            config.pad_id,
            config.pad_id,
            config.pad_id,
    ...
    def _make_vocab(self, config):
            ...
            for item in data:
                ...
                for token in token_list:
                    ...
                    if is_vn:
                        ...
                        if medial not in self.specials:
                            counter_medial.update([medial])
                        if nucleus not in self.specials:
                        ...
                        for char in token:
                            (
                                ...
                                nucleus_char,
                                coda_char,
                            ) = split_non_vietnamese_word(char)
                            ...
                            if medial_char not in self.specials:
                                counter_medial.update([medial_char])
                            if nucleus_char not in self.specials:
                                counter_nucleus.update([nucleus_char])
                            if coda_char not in self.specials:
                                counter_coda.update([coda_char])
        ...
        sorted_medial = sorted(counter_medial)
        sorted_nucleus = sorted(counter_nucleus)
        sorted_coda = sorted(counter_coda)
        ...
        self.stoi_medial = {
            tok: i for i, tok in enumerate(self.specials + sorted_medial)
        }
        self.itos_nucleus = {
            i: tok for i, tok in enumerate(self.specials + sorted_nucleus)
        }
        self.stoi_nucleus = {
    ...
    def encode_sequence_labeling(
        ...
        for word_idx, word in enumerate(words):
            ...
            if is_vn:
                ...
                medial_idx = self.stoi_medial.get(medial, self.unk_idx[2])
                nucleus_idx = self.stoi_nucleus.get(nucleus, self.unk_idx[3])
                coda_idx = self.stoi_coda.get(coda, self.unk_idx[4])
                # rhyme_idx = self.stoi_rhyme.get(rhyme, self.unk_idx[2])
                input_ids.append(
                    (onset_idx, tone_idx, medial_idx, nucleus_idx, coda_idx)
                )
                vec.append((onset_idx, tone_idx, medial_idx, nucleus_idx, coda_idx))

            else:
                # For non-Vietnamese, we mark a "space" quintuplet, then each char
                # individually, then a closing "space" quintuplet
                ...
                for char in word:
                    ...
                    m_idx = self.stoi_medial.get(medial_c, self.unk_idx[2])
                    n_idx = self.stoi_nucleus.get(nucleus_c, self.unk_idx[3])
                    c_idx = self.stoi_coda.get(coda_c, self.unk_idx[4])
                    # r_idx = self.stoi_rhyme.get(rhyme_c, self.unk_idx[2])
                    vec.append((o_idx, t_idx, m_idx, n_idx, c_idx))
    ...
    def align_labels_with_subwords(
        self, labels: List[str], word_to_subword_mapping: List[int]
    ) -> List[str]:
        """
        ...
        for subword_idx in word_to_subword_mapping:
            if subword_idx < len(labels):
                ...
                print(
                    f"Warning: subword_idx ({subword_idx}) exceeds labels length ({len(labels)})"
                )
    ...
    def decode_sentence(self, encoded_sentence: torch.Tensor) -> str:
        ...
        while i < len(encoded_sentence):
            ...
            medial = self.itos_medial.get(medial_idx, "")
            nucleus = self.itos_nucleus.get(nucleus_idx, "")
            coda = self.itos_medial.get(coda_idx, "")
            # rhyme = self.itos_rhyme.get(rhyme_idx, "")

            # BOS/EOS check (if needed)
            if (onset, tone, medial, nucleus, coda) == (
                self.bos_token,
                self.bos_token,
                self.bos_token,
                self.bos_token,
            ...
            if (onset, tone, medial, nucleus, coda) == (
                ...
                current_word = []
                while i < len(encoded_sentence):
                    on_idx, to_idx, me_idx, nu_idx, co_idx = encoded_sentence[
                        i
                    ].tolist()
                    on_str = self.itos_onset.get(on_idx, "")
                    to_str = self.itos_tone.get(to_idx, "")
                    me_str = self.itos_medial.get(me_idx, "")
                    nu_str = self.itos_nucleus.get(nu_idx, "")
                    co_str = self.itos_coda.get(co_idx, "")

                    # rh_str = self.itos_rhyme.get(rh_idx, "")
                    if (on_str, to_str, me_str, nu_str, co_str) == (
                        self.space_token,
                        self.space_token,
                        self.space_token,
                        self.space_token,
            ...
            word = self._recompose_word(onset, tone_char, medial, nucleus, coda)
            words.append(word)
            i += 1
    ...
    def total_tokens_dict(self) -> dict:
        ...
        return {
            ...
            "medial": len(self.itos_medial),
            "nucleus": len(self.itos_nucleus),
            "coda": len(self.itos_coda),
            "all": (
                len(self.itos_onset)
                + len(self.itos_tone)
                + len(self.itos_medial)
                + len(self.itos_nucleus)
                + len(self.itos_coda)
            ),
        }
